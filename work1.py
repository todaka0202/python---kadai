...
def button_action1():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()  # 入力値を取得
    user_input2 = entry2.get()
    kekka1 = int(user_input1) + int(user_input2)
    label1.config(text=f"{user_input1} + {user_input2} = {kekka1}")  # 画面に出力

def button_action2():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()  # 入力値を取得
    user_input2 = entry2.get()
    kekka2 = int(user_input1) - int(user_input2)
    label1.config(text=f"{user_input1} - {user_input2} = {kekka2}")  # 画面に出力

def button_action3():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()  # 入力値を取得
    user_input2 = entry2.get()
    kekka3 = int(user_input1) * int(user_input2)
    label1.config(text=f"{user_input1} × {user_input2} = {kekka3}")  # 画面に出力

def button_action4():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()  # 入力値を取得
    user_input2 = entry2.get()
    kekka4 = int(user_input1) / int(user_input2)
    label1.config(text=f"{user_input1} ÷ {user_input2} = {kekka4}")  # 画面に出力
